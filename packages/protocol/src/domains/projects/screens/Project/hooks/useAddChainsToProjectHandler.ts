...
import { selectProjectWhitelistBlockchains } from 'domains/projects/store/WhitelistsSelector';
...
export const useAddChainsToProjectHandler = ({
  ...
  const { project, userEndpointToken } = useSelectedProject();

  const projectBlockchainsBeforeEdit = useAppSelector(
