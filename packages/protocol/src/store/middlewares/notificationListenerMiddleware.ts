...
import { INotificationProps, notificationSlice } from 'domains/notification/store/notificationSlice';

const NOTIFICATION_AUTO_HIDE_DURATION = 2_000;

export const notificationListenerMiddleware =
...
notificationListenerMiddleware.startListening({
  ...
  effect: async (
    ...
    dispatch(notificationSlice.actions.pushNotificationToTheQueue(notification));

    // in case if autoHideDuration is equal to null, 
    // notification shouldn't be closed automatically
    if (autoHideDuration !== null) {
      setTimeout(() =>
        dispatch(notificationSlice.actions.hideNotification(notification.key)),
