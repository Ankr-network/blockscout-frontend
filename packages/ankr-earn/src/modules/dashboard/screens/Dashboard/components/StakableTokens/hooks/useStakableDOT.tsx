...
export const useStakableDOT = (): IUseStakableToken<IPolkadotNetwork> => {
  ...
  const networksData = useMemo(
    () =>
      networks.filter(({ chainId }) => chainId === EPolkadotNetworkId.polkadot),
    [networks],
  );

  const apy = metrics ? +metrics.dot.apy : 0;
  const balance = polkadotBalance ?? ZERO;

  return {
    apy,
    balance,
    href: StakePolkadotRoutes.stake.generatePath(EPolkadotNetworks.DOT),
    icon: <DotIcon />,
