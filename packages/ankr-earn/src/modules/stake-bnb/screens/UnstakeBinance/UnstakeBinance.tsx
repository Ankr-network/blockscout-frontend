...
import { getAnkrBalance } from 'modules/stake-bnb/actions/getAnkrBalance';
import {
  IUnstakeFormValues,
  UnstakeDialog,
...
import { useUnstakeBinanceStyles as useStyles } from './useUnstakeBinanceStyles';
        ...
        >
          {({ data: statsData, loading }, { data: ankrBalance }) => (
            <UnstakeDialog
              ...
              balance={statsData.aBNBbBalance}
              onClose={onClose}
              onSubmit={onUnstakeSubmit}
              endText={t('stake-bnb-dashboard.unstake-eta')}
              token={Token.aBNBb}
              extraValidation={(data, errors) => {
                if (ankrBalance.isLessThan(statsData.unstakeFee)) {
                  errors.amount = tHTML(
                    'stake-bnb-dashboard.validation.is-not-enough-fee',
                    {
