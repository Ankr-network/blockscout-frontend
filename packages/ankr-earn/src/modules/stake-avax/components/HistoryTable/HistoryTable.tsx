...
function getPreparedData(data?: TTxEventsHistoryGroupData): TTableData {
  ...
  return data.map(({ txAmount, txDate, txHash, txType }) => ({
    amount: t('unit.avax-value', { value: txAmount.toNumber() }),
...
export const HistoryTable = ({
  ...
  const switcherCaptions = useLocaleMemo(
    () => [
      {
        label: t('stake-avax-dashboard.switcher.completed-title'),
      },
      {
        label: t('stake-avax-dashboard.switcher.pending-title'),
      },
    ],
    [],
  );
  ...
  const headerCaptions = useLocaleMemo(
    () => [
      {
        label: t('stake-avax-dashboard.column.tx-date'),
      },
      {
        label: t('stake-avax-dashboard.column.tx-hash'),
      },
      {
        label: t('stake-avax-dashboard.column.tx-type'),
      },
      {
        align: 'right',
