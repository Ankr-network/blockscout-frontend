import { t } from '@ankr.com/common';
import BigNumber from 'bignumber.js';

import { getExtendedErrorText } from 'modules/api/utils/getExtendedErrorText';
import { queryFnNotifyWrapper, web3Api } from 'modules/api/web3Api';
import { ACTION_CACHE_SEC, WalletCacheTags } from 'modules/common/const';

import { CacheTags as AvaxCacheTags } from '../const';
import { getAvalancheSDK } from '../utils/getAvalancheSDK';

export const { useGetAvaxPendingUnstakesQuery } = web3Api.injectEndpoints({
  endpoints: build => ({
    getAvaxPendingUnstakes: build.query<BigNumber, void>({
      queryFn: queryFnNotifyWrapper<void, never, BigNumber>(
        async () => {
          const sdk = await getAvalancheSDK();
          const data = await sdk.getPendingClaim();

          return { data };
        },
        error =>
          getExtendedErrorText(error, t('stake-avax.errors.pending-values')),
      ),
      keepUnusedDataFor: ACTION_CACHE_SEC,
      providesTags: [AvaxCacheTags.common, WalletCacheTags.account],
    }),
  }),
});
