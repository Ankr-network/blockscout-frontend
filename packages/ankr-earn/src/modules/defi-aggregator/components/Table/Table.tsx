...
export const Table = ({ data }: ITableProps): JSX.Element => {
  ...
  const sortedData = useMemo(() => {
    ...
    return data.sort((a, b) => {
      if (sortOrder === 'asc') {
        return a[sortKey as keyof DeFiItem].localeCompare(
          b[sortKey as keyof DeFiItem],
        );
      }
      return b[sortKey as keyof DeFiItem].localeCompare(
        a[sortKey as keyof DeFiItem],
      );
    });
  }, [data, sortKey, sortOrder]);

  const renderTableHeadCell = (title: string) => {
    const isSelected = sortKey === title;
